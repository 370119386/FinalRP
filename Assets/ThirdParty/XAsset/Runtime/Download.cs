...
namespace VEngine
{
    ...
    public class Download : CustomYieldInstruction
    {
        ...
        ///     下载失败的队列
        /// </summary>
        public static readonly List<Download> Errors = new List<Download>();
        ...
        public bool isDone => status == DownloadStatus.Failed || status == DownloadStatus.Success || status == DownloadStatus.Canceled;
        ...
        // private int lastExceptionTime;
        // private static void TreatException()
        // {
        //     if (true)
        //     {
        //         throw new Exception("TreatException");
        //     }
        // }
        ...
        {
            if (retryEnabled && retryTimes < MaxRetryTimes)
            {
            ...
            using (var response = request.GetResponse())
            {
                if (response.ContentLength > 0)
                {
                    ...
                    using (var reader = response.GetResponseStream())
                    {
                        ...
                        //lastExceptionTime = DateTime.Now.Millisecond;
                        while (status == DownloadStatus.Progressing)
                        {
                            ...
                            // if (DateTime.Now.Millisecond - lastExceptionTime > 30 && !retryEnabled)
                            // {
                            //     lastExceptionTime = DateTime.Now.Millisecond;
                            //     TreatException();
                            // }
